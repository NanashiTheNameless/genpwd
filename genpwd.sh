...
for ((i=1; i<=times_to_run; i++)); do
    ...
    if [ "$xtra" = "true" ]; then
      ...
      # Check if cowsay is installed
          ...
          echo ""
          echo "$(cowsay $(tr -cd "[:graph:]" < /dev/urandom | head -c $(($min_word_length + 20)) | sed -e 's|\`|~|g' -e 's|\$(|\\$(|g' -e 's|~||g';))"
          echo ""
      else
          echo -e "$(tr -cd "[:graph:]" < /dev/urandom | head -c $(($min_word_length + 20)) | sed -e 's|\`|~|g' -e 's|\$(|\\$(|g' -e 's|~||g';)"
      fi
